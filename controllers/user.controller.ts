...
import { getLoggedInUsersService } from "../services/user.service";
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      try {
        ...
        res.status(201).json({
          success: true,
          message: `Check yout main: ${user.email} to activate account`,
...
export const updateAccessToken = CatchAsyncError(
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const user = JSON.parse(session); // stored in redis
...
export const getLoggedInUsers = async (req: Request, res: Response) => {
  await getLoggedInUsersService(res);
};
